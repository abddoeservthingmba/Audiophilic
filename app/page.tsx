...
import { Search, Music2, X, Sparkles, Flame, Radio } from 'lucide-react'
    ...
    <>
      ...
      <main className="min-h-screen pb-28">
        ...
        <div className="sticky top-0 z-40 backdrop-blur-xl bg-black/60 border-b border-white/8">
          ...
          {/* Genre Tabs */}
          {!searchMode && (
            <div className="max-w-7xl mx-auto px-4 pb-2.5 overflow-x-auto no-scrollbar flex gap-2 pt-1">
              {GENRES.map((g) => (
                <button
                  key={g}
        ...
        {/* Hero Quick Picks */}
        {!searchMode && activeGenre === 'All' && !loading && tracks.length > 0 && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {tracks.slice(0, 6).map((t, idx) => (
                <div
                  ...
                  className="flex items-center gap-3 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl p-2 cursor-pointer transition-all hover:scale-[1.01] group"
                >
